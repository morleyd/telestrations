<template>
  <v-card class="overflow-y-auto pa-0 align-content-space-evenly" height="100vh" width="100vw" color="background" style="justify-items: center;
    display: grid
;">
            ...
            <template v-slot:default="{ isActive }">
              <v-card class="pa-4 bg-white" width="500" max-width="100%">
                ...
                <v-form ref="form" @submit.prevent="onBeginClicked">
                  <v-row class="pa-2">
                    <v-text-field v-model="username" label="Username" @input="username = username.toLowerCase()"
                       :rules="[v => v && v.trim || 'Username cannot be empty!']" />
                  </v-row>
                ...
                <v-form ref="form" @submit.prevent="onJoinClicked">
                  ...
                  <v-row class="pa-2">
                    <v-text-field v-model="username" label="Userame" @input="username = username.toLowerCase()"
                      @keyup.enter="onJoinClicked" :rules="[v => v && v.trim || 'Userame cannot be empty!']" />
                  </v-row>
                  <v-row class="pa-2" style="justify-content: center;">
...
export default {
  ...
  methods: {
    async onJoinClicked() {
      ...
      let user = await pbService.users.getUser(this.username, validGame.gameId)
      if (user.hasOwnProperty("id")) {
        this.userStore.user = user
        this.$emit("snack", "Username already exists. Assuming it's yours.", "warning")
      } else {
        let error = await this.userStore.newUser(this.username, validGame.gameId, false)
        if (error) {
    ...
    async onBeginClicked() {
      ...
      let resp = await this.userStore.newGame(this.username, time)
      if (resp.errMsg) {
